...
numFiles <- 11			# Edit this number based on the number of rarefactions
...
for (j in 1: numAnalysis)
{
	...
	minRange <- min(ref.plot)
	maxRange <- max(ref.plot)
	...
	for (i in 2:numFiles)
	{
		...
		# Compute range of V1 and Vi
		minRange <- if(minRange < min(new.plot)) min(new.plot) else minRange
		maxRange <- if (maxRange > max(new.plot)) max(new.plot) else maxRange
...
colours <- c(rgb(1,0,0,0.2), rgb(0,1,0,0.2), rgb(0,0,1,0.2), rgb(0,0,0,0.2) )
ellipseColours <- c(rgb(1,0,0,1), rgb(0,1,0,1), rgb(0,0,1,1), rgb(0,0,0,1) )
...
# GENERATES ELLIPSES FROM THE DATA 
pdf("MRD_vs_MD_Tongue_dorsum_100_ellipse.pdf")

# This script is used to generate the plots of the Max Relative Deviation vs
# Median Deviation with 95% confidence ellipses
...
colours <- c(rgb(1,0,0,pointDarkness), rgb(0,1,0,pointDarkness), rgb(0,0,1,pointDarkness), rgb(0,0,0,pointDarkness) )
ellipseColours <- c(rgb(1,0,0,elipseDarkness), rgb(0,1,0,elipseDarkness), rgb(0,0,1,elipseDarkness), rgb(0,0,0,elipseDarkness) )
...
for (i in 1:numAnalysis)
{
	x <- as.numeric(listPlot[[i]][2:numFiles,1])
	m <- mean(x)
	x <- as.numeric(listPlot[[i]][,1])
	x[1] <- m
	y <- as.numeric(listPlot[[i]][2:numFiles,2])
	m <- mean(y)
	y <- as.numeric(listPlot[[i]][,2])
	y[1] <- m

	# Function to draw ellipse	
	dataEllipse(x,y,xlim=c(0,xMax), ylim=c(0,yMax), plot.points=FALSE, 
	#col=rgb(0,0,0,1), levels=c(0.95), center.pch = 0)
	col = ellipseColours[i], levels=c(0.95), center.pch=0)
