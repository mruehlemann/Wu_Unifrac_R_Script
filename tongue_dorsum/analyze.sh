...
R --slave --no-save --args $subsite $subsamples < generate_aitchison_data.R
...
# Save the file for TCC normalization
readsR=$(cat $dir$red$und$subsite$und$subsamples$ext)
echo -en "\t$readsR" > $dir$red$und$subsite$und$subsamples$und$ar$und$ext

# Save the OTU table for Qiime Processing
...
R --slave --no-save --args $DEP $subsite < max_vs_med_plot.R
