...
for (i in c(1:length(taxonomy))) {
  taxonomy[i] <- paste(strsplit(taxonomy[i],c(";"))[[1]][6],strsplit(taxonomy[i],c(";"))[[1]][7],sep="")
}
taxonomy <- paste(taxonomy,c(1:length(taxonomy)),sep="_")
...
colnames(monocultures) <- colnames(breastmilk.otu.tab)
for (i in c(1:60)) {
...
for (i in c(1:20)) {
  monocultures[i,which(monocultures[i,]==max.otu)] <- monocultures[i,1]
  monocultures[i,1] <- max.otu
}
for (i in c(21:40)) {
  monocultures[i,which(monocultures[i,]==max.otu)] <- monocultures[i,2]
  monocultures[i,2] <- max.otu
}
for (i in c(41:60)) {
  monocultures[i,which(monocultures[i,]==max.otu)] <- monocultures[i,3]
  monocultures[i,3] <- max.otu
}

#flip table so that sample names are in column headers, to be consistent with other data sets
monocultures <- t(monocultures)
monocultures <- data.frame(monocultures)
monocultures$taxonomy <- taxonomy
